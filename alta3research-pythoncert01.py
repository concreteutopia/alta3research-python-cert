...
def main():
    ...
    while True:
        idlist=[]
        randomlist=["warhol","van gogh","picasso","wirsum","gladys nilsson","frank stella","diego rivera","ancient egyptian","aztec","edward hopper","cats","hokusai","willem de kooning","jackson pollock",
                "david hockney","takashi murakami","lichtenstein","christopher wool","pop art","keith haring","alexander calder","contemporary art","modern art","gerhard richter","lighthouse"]

        #Input search query or choose a random search from randomlist if no search is provided.
        print("Enter artist, artwork or phrase to search for.")
        print("Your query will search the Art Institute of Chicago's collection for an artist, artwork, phrase or keyword.\n(ex: Picasso, Andy Warhol, cats, Van Gogh, Monet, Chicago)")
        search=input("If you do not know what to search for, simply press <enter> and a random search will be made for you. (Type 'q' to quit) >> ")
        ...
        #Parse the search results, grab the artwork id and put it in a list.  We need the artwork id to query the artworks api to get the full artwork details
        for resultsid in artistresults.json().get("data"):
            idlist.append(resultsid['id'])
        clear()
        #For every id in the list, display the artwork details and ask if the artwork image should be downloaded.
        for artid in idlist:
            try:
            ...
            if artimageid is not None:
                ...
                if download.lower() == "q" or download.lower()=="quit":
                    #a break here returns us to the top of the while loop to the search query rather than exiting the script
                    break
                elif download.lower() == "y" or download.lower()=="yes":
                    try:
                        #use requests to download the artwork image and then write it to a file, trap errors
                    ...
                    #continue will skip, go to the top of the for loop and move on to the next artwork in the id list
                    continue
                else:
